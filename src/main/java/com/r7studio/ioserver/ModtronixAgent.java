 ...
 */
@Path("/modtronix")
public class ModtronixAgent extends Thread {
    ...
    @GET
    @Path("/rollLCD")
    public void rollLCD(@QueryParam("msg") String msg) {
        try {
            Thread.sleep(200);
    ...
    public synchronized static void sendConf(byte[] message) {
        ...
        try {
            ...
            Thread.sleep(35); //allow the LCD to proccess the data before sending the next command
            ...
            try {
                ...
                for (String event : events) {
                    ...
                    if (event.startsWith("l34=")) {
                        String key = event.substring(4, 5);
                        IOServer.onI2CKey(Integer.parseInt(key.toUpperCase(), 16), incoming.getAddress());
                    }
